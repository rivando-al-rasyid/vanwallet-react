          ...
          <div className="mb-6 flex items-center gap-2">
            {[1, 2, 3].map((n) => (
              <div key={n} className="flex items-center gap-2">
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-xs font-black text-primary-content">{n < 3 ? "✓" : n}</div>
                {n < 3 && <div className="h-px w-8 bg-base-300" />}
              </div>
            ))}
            <span className="ml-2 text-xs font-bold text-base-content/50">New Password</span>
          </div>
          {success ? (
            <div className="flex flex-col items-center gap-4 py-8 text-center">
              <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-success/10 text-success">✓</div>
              <h2 className="text-xl font-black text-base-content">Password Changed!</h2>
              <p className="text-sm text-base-content/65">Your password has been updated successfully. Redirecting to login...</p>
            </div>
          ) : (
            <>
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                {form.confirmPassword.length > 0 && form.newPassword !== form.confirmPassword && <p className="text-xs font-bold text-error">Passwords do not match.</p>}
