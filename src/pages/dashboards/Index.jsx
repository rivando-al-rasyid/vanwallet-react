...
export default function Index() {
  ...
  const chartData = useMemo(() => {
    ...
    if (typeFilter === "All" || typeFilter === "Income") {
      datasets.push({ label: "Income", data: income, backgroundColor: "#4D7CFF", borderRadius: 10, borderSkipped: false, barPercentage: 0.45 });
    }
    if (typeFilter === "All" || typeFilter === "Expense") {
      datasets.push({ label: "Expense", data: expense, backgroundColor: "#CC0000", borderRadius: 10, borderSkipped: false, barPercentage: 0.45 });
    }
  ...
  const options = {
    responsive: true,
    plugins: {
      legend: { position: "bottom", labels: { usePointStyle: true, pointStyle: "circle", padding: 32, font: { size: 15, weight: "500" } } },
      tooltip: { callbacks: { label: (ctx) => `Rp ${ctx.parsed.y.toLocaleString("id-ID")}` } },
    },
    scales: {
      x: {
        offset: true,
        grid: { drawOnChartArea: false, color: "rgba(0,0,0,0.06)", drawTicks: true, tickLength: 15, tickColor: "#000", offset: false },
        border: { display: false },
        ticks: { color: "#64748b", font: { size: 13 }, padding: 12, maxRotation: 0, minRotation: 0 },
      },
      y: {
        beginAtZero: true,
        border: { display: false },
        ticks: { stepSize: 25000, font: { family: "Montserrat", size: 11 } },
        grid: { color: "rgba(0,0,0,0.06)", drawTicks: true, tickLength: 7, tickColor: "#000" },
      },
    },
  };
      ...
      <div className="flex flex-col gap-4 lg:gap-5">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 items-start">
          ...
          <div className="fade-in delay-1 card flex flex-col gap-4">
            ...
            <div>
              <p className="text-2xl lg:text-3xl font-extrabold text-slate-800 tracking-tight">Rp.120.000</p>
            </div>
            <div className="flex gap-6">
              <div>
                <p className="text-[10px] uppercase tracking-wider text-slate-400 mb-1 font-bold">Income</p>
              ...
              <div>
                <p className="text-[10px] uppercase tracking-wider text-slate-400 mb-1 font-bold">Expense</p>
          ...
          <div className="rounded-lg w-full">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
              ...
              <div className="flex gap-2 sm:gap-3">
                <select value={days} onChange={(e) => setDays(e.target.value)} className="text-xs font-semibold text-slate-600 border border-slate-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-200">
                  <option value="7">7 Days</option>
                  <option value="14">14 Days</option>
                  <option value="30">30 Days</option>
                </select>
                <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="text-xs font-semibold text-slate-600 border border-slate-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-200">
