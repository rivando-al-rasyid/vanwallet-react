...
export default function ChangePassword() {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.currentPassword || !form.newPassword || !form.confirmNewPassword) return setLocalError("Semua field password wajib diisi.");
    if (form.newPassword.length < 8) return setLocalError("Password baru minimal 8 karakter.");
    if (form.newPassword !== form.confirmNewPassword) return setLocalError("Konfirmasi password baru tidak cocok.");
    if (form.currentPassword === form.newPassword) return setLocalError("Password baru tidak boleh sama dengan password lama.");
    const result = await dispatch(changePassword({ oldPassword: form.currentPassword, newPassword: form.newPassword }));
  ...
  const EyeIcon = ({ visible }) => visible ? (
    ...
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17.94 17.94A10.94 10.94 0 0112 20C7 20 2.73 16.11 1 12c.92-2.19 2.49-4.08 4.5-5.5" /><path d="M10.58 10.58A2 2 0 0012 14a2 2 0 001.42-.58" /><path d="M9.88 5.09A10.94 10.94 0 0112 4c5 0 9.27 3.89 11 8a11.83 11.83 0 01-4.24 5.11" /><path d="M1 1l22 22" /></svg>
  );
  const LockIcon = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="10" rx="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></svg>);
  const fields = [
  ...
  const confirmMismatch = form.confirmNewPassword.length > 0 && form.newPassword !== form.confirmNewPassword;
