...
export default function Hero() {
  return (
    <header className="relative overflow-hidden bg-neutral text-neutral-content">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_0.9fr] lg:px-10 lg:py-24">
        <div className="max-w-3xl">
          <span className="inline-flex rounded-full border border-neutral-content/20 bg-neutral-content/10 px-4 py-2 text-xs font-black uppercase tracking-[0.25em] text-success backdrop-blur">Digital wallet</span>
          ...
          <p className="mt-6 max-w-xl text-base leading-8 text-neutral-content/70 sm:text-lg">
...
function PhoneMockup() {
  return (
    <div className="relative flex justify-center lg:justify-end">
      ...
      <div className="relative w-fit rounded-[3rem] border border-neutral-content/20 bg-neutral-content/10 p-8 shadow-2xl backdrop-blur">
...
function DownloadButtons() {
  return (
    <div className="mt-8 mb-8 flex flex-col gap-3 sm:flex-row sm:gap-4 lg:mb-10">
      <button className="flex w-full min-w-42 items-center justify-center gap-3 rounded-2xl bg-base-100 px-6 py-3 font-bold text-primary shadow-lg transition hover:-translate-y-0.5 hover:bg-success/10 sm:w-auto">
        <FontAwesomeIcon icon={faGooglePlay} height={ICON_SIZE} width={ICON_SIZE} /><span>Play Store</span>
      </button>
      <button className="flex w-full min-w-42 items-center justify-center gap-3 rounded-2xl border border-neutral-content/20 px-6 py-3 font-bold text-neutral-content transition hover:bg-neutral-content/10 sm:w-auto">
    ...
    <div className="flex items-center gap-5">
      ...
      <div>
        <div className="mb-2 flex -space-x-3">
          {AVATAR_IDS.map((id) => (
            <img key={id} src={`https://i.pravatar.cc/100?img=${id}`} alt="User avatar" className="h-10 w-10 rounded-full border-2 border-neutral object-cover" />
          ))}
        </div>
        <p className="max-w-xs text-sm leading-6 text-neutral-content/70">Happy users already trust the wallet for everyday payments.</p>
      </div>
    </div>
  );
}
